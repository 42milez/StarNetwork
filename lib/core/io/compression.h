...
class Compression
{
  public:
    ...
    static int
    compress(std::vector<uint8_t> &dst, std::vector<uint8_t> &src, Compression::Mode mode);
    static int
    decompress(std::vector<uint8_t> &dst, int dst_max_size, std::vector<uint8_t> &src, int src_max_size, Mode mode);
    static int
    get_max_compressed_buffer_size(int p_src_size, Mode p_mode);
};
