...
if [[ ${REPORTER} = "junit" ]]; then
  ...
  "${CMAKE_BUILD_DIR}"/bin/babroadcast_test           -r junit -d yes --order lex -o "${TEST_RESULT_DIR}/e2e_babroadcast_test.xml"
  ...
  "${CMAKE_BUILD_DIR}"/bin/babroadcast_test           -r console -d yes --order lex
